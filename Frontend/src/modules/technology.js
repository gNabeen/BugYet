...
const Technology = () => {
  ...
  const checkReachability = () => {
    ...
    domainArray.forEach((domain, index) => {
      axios
        .post("https://bugyet.repl.co/technology", { domain })
        .then((response) => {
  ...
  function saveTaskCount(count) {
    localStorage.setItem("TechnologyTaskCount", parseInt(count));
  }

  // Retrieve the number of times the task has been run
  function getTaskCount() {
    return parseInt(localStorage.getItem("TechnologyTaskCount") || 0);
  }
    ...
    <div className="container mt-5">
      <Form className="search-form" onSubmit={handleSubmit}>
        ...
        <button type="submit" className="btn btn-primary" disabled={isLoading}>
          {isLoading ? "Checking..." : "Techonology Scan"}
        </button>
      </Form>
              ...
              currentResults.map((result, index) => (
                <tr key={index}>
                  ...
                  <td>
                    {result.technologies && (
                      <ul style={{ listStyleType: "none" }}>
                        {result.technologies.split("+").map((tech, index) => (
                  ...
                  <td>
                    {result.types && (
                      <ul style={{ listStyleType: "none" }}>
                        {result.types.split("+").map((type, index) => (
...
/* Exporting the Technology class. */
